...
async def show_barber_statistics(message: types.Message):
    ...
    barber_key = str(barber.id)  # MUHIM
    today = date.today()
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="Barcha buyurtmalar 📂", callback_data="barberpanel_all_orders_page_1")]]
    )
...
def _build_pagination_keyboard(page: int, total_pages: int):
    if total_pages <= 1:
        return None
    row = []
    if page > 1:
        row.append(
            InlineKeyboardButton(
                text="⬅️ Oldingi", callback_data=f"barberpanel_all_orders_page_{page - 1}"
            )
        )
    if page < total_pages:
        row.append(
            InlineKeyboardButton(
                text="Keyingi ➡️", callback_data=f"barberpanel_all_orders_page_{page + 1}"
            )
        )
